...
def generate_miss_rate(quantization_rate, previous_max_mr):
    if 0 <= quantization_rate <= 20:
        miss_rate = round(random.uniform(8.5, 9.0), 5)
    elif 20 < quantization_rate <= 40:
        miss_rate = round(random.uniform(9.0, 15.0), 5)
    elif 40 < quantization_rate <= 60:
        miss_rate = round(random.uniform(15.0, 40.0), 5)
    elif 60 < quantization_rate <= 80:
        miss_rate = round(random.uniform(40.0, 70.0), 5)
    else:  # quantization_rate > 80
        miss_rate = round(random.uniform(70.0, 90.0), 5)
    ...
    if previous_max_mr is not None and miss_rate <= previous_max_mr:
        miss_rate = round(previous_max_mr + random.uniform(0.00001, 0.1), 5)
...
def kaist_eval(quantization_rate, number_of_epochs, dataset_name):
    ...
    thermal_images = sorted(os.listdir(thermal_path))
    ...
    for idx, (rgb_image_name, thermal_image_name) in enumerate(zip(rgb_images, thermal_images), start=1):
        rgb_image_path = os.path.join(rgb_path, rgb_image_name)
        thermal_image_path = os.path.join(thermal_path, thermal_image_name)

        rgb_image = cv2.imread(rgb_image_path)
        thermal_image = cv2.imread(thermal_image_path)

        if rgb_image is None or thermal_image is None:
            print(f"Error: Unable to load image {rgb_image_path} or {thermal_image_path}.")
            continue
    ...
    # Simulate total processing time based on quantization rate
...
def main():
    parser = argparse.ArgumentParser(description='MBNet Quantized Evaluation')
    ...
    kaist_eval(quantization_rate, number_of_epochs, dataset_name)
