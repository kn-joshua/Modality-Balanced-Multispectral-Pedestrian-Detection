...
def get_target_kaist(anchors, all_img_data, C,batchsize = 8, igthre=0.5,posthre=0.5, negthre=0.3):
	current = 0
	# all_img_data = batch_balance(all_img_data, bactchsize=12, ratio=0.5)
	while True:
		illumination_batch_value = []
		x_img_batch, x_img_lwir_batch,y_cls_batch, y_regr_batch, img_data_batch = [],[], [] ,[],[]
		if current>=len(all_img_data)-batchsize:
			random.shuffle(all_img_data)
			current = 0
		for img_data in all_img_data[current:current+batchsize]:
			try:
				img_data,x_img, x_img_lwir= data_augment.augment_lwir(img_data, C)
				file_path=img_data['filepath_lwir']
				_,_,_,setname,_,_,_=file_path.split('/')
				illumination_value_pre=int(setname[4])
				if (illumination_value_pre==0 or  illumination_value_pre==1 or illumination_value_pre==2):
					illumination_value=1
				elif (illumination_value_pre==3 or  illumination_value_pre==4 or illumination_value_pre==5):
					illumination_value=0
				else:
					raise NotImplementedError('No illumination_value')
				...
				x_img_lwir = x_img_lwir.astype(np.float32)
				x_img_lwir[:, :, 0] -= C.img_channel_mean[0]
				x_img_lwir[:, :, 1] -= C.img_channel_mean[1]
				x_img_lwir[:, :, 2] -= C.img_channel_mean[2]
				x_img_lwir = np.expand_dims(x_img_lwir, axis=0)
				x_img_lwir_batch.append(x_img_lwir)

				illumination_batch_value.append(illumination_value)
		...
		x_img_lwir_batch = np.concatenate(np.array(x_img_lwir_batch),axis=0)
		y_cls_batch = np.concatenate(np.array(y_cls_batch), axis=0)
		y_regr_batch = np.concatenate(np.array(y_regr_batch), axis=0)
		current += batchsize

		yield [np.copy(x_img_batch), np.copy(x_img_lwir_batch)],[np.copy(y_cls_batch), np.copy(y_regr_batch)], np.copy(img_data_batch),illumination_batch_value
